...
func WithReadBufferLen(len int) Option {
	return newFuncServerOption(func(o *options) {
		if len <= 0 {
			util.Panic("acceptGNum must greater than 0")
		}
		o.readBufferLen = len
	})
}
...
func WithIOGNum(num int) Option {
	return newFuncServerOption(func(o *options) {
		if num <= 0 {
			util.Panic("IOGNum must greater than 0")
		}
		o.ioGNum = num
	})
}
...
func WithIOEventQueueLen(num int) Option {
	return newFuncServerOption(func(o *options) {
		if num <= 0 {
			util.Panic("ioEventQueueLen must greater than 0")
		}
...
func WithTimeout(timeout time.Duration) Option {
	return newFuncServerOption(func(o *options) {
		if timeout <= 0 {
			util.Panic("timeoutTicker must greater than 0")
		}
